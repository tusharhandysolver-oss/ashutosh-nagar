...
export default function CalendarView({ tasks, users, events, onSelectTask, onCreateEvent }: Props) {
  ...
  return <div className="space-y-3 md:space-y-4 min-w-0">
    ...
    <section className="calendar-shell flex flex-col bg-white border border-slate-200 rounded-2xl lg:h-[calc(100dvh-205px)] lg:min-h-[500px] lg:max-h-[720px] shadow-[0_20px_60px_-42px_rgba(15,23,42,.45)]" aria-label={`${monthName} ${year} calendar`}>
      ...
      <div className="calendar-grid grid flex-1 grid-cols-7 min-h-0" style={{ gridTemplateRows: `repeat(${cells.length / 7}, minmax(0, 1fr))` }}>
        {cells.map(({ date, inMonth }, index) => <div key={index} style={{ animationDelay: `${Math.min(index, 20) * 12}ms` }} className={`calendar-day calendar-cell-enter min-h-[76px] max-h-[150px] lg:min-h-0 lg:max-h-full overflow-y-auto p-1 sm:p-1.5 space-y-0.5 min-w-0 border-r border-b ${inMonth ? "bg-white border-slate-100" : "bg-slate-50/60 border-slate-100/70"}`}>
