    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      <aside className="lg:col-span-5 soft-shadow bg-white border border-slate-200/60 rounded-3xl p-6 h-fit space-y-5"><div><h3 className="font-bold text-xs uppercase tracking-wider text-slate-500">{editingId ? "Edit Case" : "Add Case"}</h3><p className="text-xs text-slate-400 mt-1">Choose an existing client name to add another case under that client.</p></div>
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><label className="text-xs font-bold text-slate-500">Matter Code<input value={form.matterCode} onChange={(e) => update("matterCode", e.target.value)} className={`${fieldClass} mt-1.5`} placeholder="Auto-generated" /></label><label className="text-xs font-bold text-slate-500">Status<select value={form.status} onChange={(e) => update("status", e.target.value)} className={`${fieldClass} mt-1.5`}><option>Active</option><option>On Hold</option><option>Closed</option></select></label></div>
